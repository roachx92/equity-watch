...
Tripwires** derived **verbatim from the report's §18**. **Write the tripwires as a bullet
list, one `- **(n)** ...` line per numbered trigger** (not one dense paragraph) — this is
...
verbatim. The audit tracks these deterministically; an expired, unfired trigger is dead
weight that reads as coverage, and gets flagged for
