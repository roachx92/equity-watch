...
Use all that apply; not every question needs all sixteen.

1. **Business model & role in the value chain** — what it does, how it makes money, where it sits in the sector's stack.
2. **Recent catalysts** — the most significant recent events, each with a **date**, sourced from current news.
3. **Re-rate map** — the drivers that would expand or compress the multiple (not just move the price), each **classified by type** (fundamental / structural / sentiment-technical / macro — see Section D) and ordered by impact, with its **mechanism** (the specific discount it removes), an **estimated multiple impact with confidence tier**, and **timing window**. See Section D for the full methodology. Close with the **near-term binary gates** that resolve most of the thesis and a **net re-rate verdict** (upside re-rate potential vs. downside de-rate risk).
...
7. **Financials** — revenue growth, margins, financial health, cash / runway / financing stack. **Explicit contracted-vs-recognized split.** GAAP vs non-GAAP. **Unit Economics & Capital Stack:** demand a breakdown of unit economics (cost to scale, yield/learning-curve trajectory, gross margin trajectory). Map the full capital stack — exact debt maturities, convertible strike prices, and ongoing ATM equity dilution programs.
...
9. **Bull / base / bear** — each with a **subjective probability** and a **price range**, anchored to a stated reference price and a valuation framework.
...
*Standard adopted July 2026 (competitive-landscape depth standard, Section G, added July 2026; filing/counterparty/sector mandatory sub-agent templates, Section H, added July 2026; §16 Final Thoughts & Conclusion added July 2026, bringing the deep-dive template to 16 sections; "Your edge" (variant-perception) and "Your tripwire" (falsification/exit-trigger) gut-checks added to the §16 pre-investment checklist July 2026; the "Latest Updates" workflow (Section F) now mandatorily assesses each news item against the §16 Tripwire and Edge, with [TRIPWIRE]/[EDGE+]/[EDGE−] tagging, July 2026). Applied to every ticker unless you override it for a specific request.*
