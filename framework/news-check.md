...
Produce a short chat-reply digest, **TRIPWIRES FIRST.** The full multi-paragraph detail lives in each ticker's `## Recent News Log` (in `tickers/<TICKER>.md`), not the chat reply — EXCEPT the Tripwires/Edge-shifts bullets below, which each get one detailed line per hit, not just a bare list:
