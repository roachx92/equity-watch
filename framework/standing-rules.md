...
- [ ] **Always dispatch four parallel sub-agents on every full run** (when sub-agents are available and the user permits) — **filing, counterparty, sector, and competitive-landscape** — then synthesize. All four are mandatory, each with a hard-coded prompt template: filing/counterparty/sector templates in **Section H**, competitive-landscape template in **Section G**. This mirrors a multi-agent research loop and materially widens source coverage. When running single-threaded, still cover all four work-streams directly.
  - **Plus a fifth, mandatory on every full run: the adversarial pass** (`adversarial-review.md` **Section L**). The four above all receive the Edge verbatim *and a hunt-list derived from it*, so they widen **source** coverage without widening **framing** coverage — the 2026-07-29 COHR run dispatched four and all four returned `[EDGE+]`. The §L red team is dispatched to **break** the Edge, is deliberately **denied that hunt-list**, must steelman the consensus and name the falsifier, and is never told the running `[EDGE±]` tally. Its findings are **reconciled, never averaged** against the other four — three agents sharing one framing and one dissenting are not four votes. **A full re-underwrite that never argued the other side has not been underwritten.**
