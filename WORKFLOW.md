...
2. **Search for recent news/headlines** on the company since the last dated entry in its Recent News Log (or the last ~14 days if the log is empty — see First-run window above), per the search budget and source-quality guidance above. Use reputable/primary sources; date every item; never fabricate — if nothing material, say so.
