...
6. End with an **abridged** run summary, **TRIPWIRES FIRST** — the full detail belongs in the ticker files (step 4), not in the chat reply: a 🚨 line listing any tripwire hits across all tickers (or "none today"), then edge shifts, then every substantive item per ticker as a headline-only line: `YYYY-MM-DD — [FRAMEWORK-TAG] — **Headline**. [TRIPWIRE/EDGE± if applicable]` (no summary/impact text — that stays in the ticker file). If a tripwire fired, lead with it.
